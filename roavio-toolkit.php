...
/**
 * Plugin Name: Roavio Toolkit
 * Description: A Helper plugin for all Roavio WordPress Themes
 * Plugin URI: #
 * Author: Webtend
 ...
 */
final class Roavio_Toolkit
{

	/**
	 * Instance of the class.
	 *
	 * @var Roavio_Toolkit|null
	 */
	...
	{
		load_plugin_textdomain('roavio-toolkit', false, plugin_basename(dirname(__FILE__)) . '/languages');
	}
	...
	{
		$theme_data = get_option('roavio_theme_verify', []);
		$active     = true;
	...
	public function admin_notice_minimum_php_version()
	{
		...
		$message = sprintf(
			...
			esc_html__('"%1$s" requires "%2$s" version %3$s or greater.', 'roavio-toolkit'),
			'<strong>' . esc_html__('Roavio Toolkit', 'roavio-toolkit') . '</strong>',
			'<strong>' . esc_html__('PHP', 'roavio-toolkit') . '</strong>',
	 ...
	 * Admin notice
	 *
	 * Warning when the site doesn't have Roavio theme installed or activated.
	 *
	 * @since 1.0.0
	 * @access public
	 */
	public function admin_notice_missing_main_theme()
	{
		...
		$message = sprintf(
			esc_html__('"%1$s" plugin requires Roavio theme to be installed and activated', 'roavio-toolkit'),
			'<strong>' . esc_html__('Roavio Toolkit', 'roavio-toolkit') . '</strong>'
		);
 ...
 * @return Roavio_Toolkit
 */
function roavio_toolkit_loading()
{
	return Roavio_Toolkit::init();
}

roavio_toolkit_loading();
