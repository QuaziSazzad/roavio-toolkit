...
class Roavio_Elementor_Extender
{
	...
	{
		add_option('roavio_update_initial_kit', 'yes');

		if ('yes' === get_option('roavio_update_initial_kit')) {
			...
			update_option('roavio_update_initial_kit', 'no');
		}
	}

	public function update_elementor_kit()
	{
		if (get_option('roavio_update_elementor_kit') === 'yes') {
			$kit = \Elementor\Plugin::$instance->kits_manager->get_active_kit();
			...
			update_option('roavio_update_elementor_kit', 'no');
		}
	}

	public function add_theme_font_group($font_groups)
	{
		$font_groups['roavio_theme_fonts'] = __('Roavio Fonts', 'roavio-toolkit');
	...
	public function add_theme_font($fonts)
	{
		...
		$fonts[$global_fonts['_primary']['font-family']]   = 'roavio_theme_fonts';
		$fonts[$global_fonts['_secondary']['font-family']] = 'roavio_theme_fonts';
	...
	public function update_options_from_elementor($check, $object_id, $meta_key, $value, $prev_value)
	{
		...
		if ($object_id == $kit_id && $meta_key == '_elementor_page_settings') {
			$options           = get_option('roavio_options');
			...
			update_option('roavio_options', $updated_options);
		}
	...
	{
		$container_width   = Roavio_Helper::get_container_max_width();
		$updated_container = [];
	...
	public function add_global_control_option($element)
	{
		...
		$element->add_control(
			'roavio_global_color_note',
			[
				'type'    => Controls_Manager::NOTICE,
				'content' => esc_html__('Please reload editor panel after updating global colors.', 'roavio-toolkit')
			]
		);
	...
	public function register_sticky_controls($element)
	{
		$element->start_controls_section(
			...
			[
				'label' => esc_html__('Roavio Sticky', 'roavio-toolkit'),
		...
		$element->add_control(
			'section_sticky_on',
			[
				'label'        => esc_html__('Enable Sticky', 'roavio-toolkit'),
				'type'         => Controls_Manager::SWITCHER,
				'return_value' => 'yes',
				'description'  => esc_html__('Make the section stick to the top when scrolling. This settings will not work with inner container', 'roavio-toolkit'),
			]
		);
		...
		$element->add_responsive_control(
			...
			[
				'label'     => esc_html__('Offset', 'roavio-toolkit'),
				'type'      => Controls_Manager::SLIDER,
				'condition' => [
					'section_sticky_on' => 'yes',
				],
				'selectors' => [
					'{{WRAPPER}}.roavio-sticky-section.roavio-sticky-active' => 'top: {{SIZE}}px;',
				],
			]
		);
		...
		$element->add_control(
			'section_sticky_bg',
			[
				'label'     => esc_html__('Active Background Color', 'roavio-toolkit'),
				'type'      => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}}.roavio-sticky-section.roavio-sticky-active' => 'background-color: {{VALUE}};',
				],
				'condition' => [
					'section_sticky_on' => 'yes',
				],
		...
		$element->add_responsive_control(
			...
			[
				'label'      => esc_html__('Active Padding', 'roavio-toolkit'),
				'type'       => Controls_Manager::DIMENSIONS,
				'size_units' => ['px', '%', 'custom'],
				'selectors'  => [
					'{{WRAPPER}}.roavio-sticky-section.roavio-sticky-active' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
				'condition'  => [
					'section_sticky_on' => 'yes',
				],
		...
		$element->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}}.roavio-sticky-section.roavio-sticky-active',
			]
		);
		...
		$element->add_responsive_control(
			...
			[
				...
				'size_units' => ['px', '%', 'custom'],
				'selectors'  => [
					'{{WRAPPER}}.roavio-sticky-section.roavio-sticky-active' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
				'condition'  => [
					'section_sticky_on' => 'yes',
				],
		...
		$element->add_group_control(
			...
			[
				'label'     => esc_html__('Active Box Shadow', 'roavio-toolkit'),
	...
	public function render_sticky_html($element)
	{
		...
		if (! empty($settings['section_sticky_on']) == 'yes') {
			$element->add_render_attribute('_wrapper', 'class', 'roavio-sticky-section');
		}
	}
	...
	public function register_tab_icon_control($element)
	{
		...
		$element->add_responsive_control(
			'roavio_icon_area_size',
			[
				'label'      => esc_html__('Area Size', 'roavio-toolkit'),
				'type'       => Controls_Manager::SLIDER,
				'size_units' => ['px', 'custom'],
				'selectors'  => [
		...
		$element->add_responsive_control(
			'roavio_icon_rotate',
			[
				'label'     => esc_html__('Icon Rotate', 'roavio-toolkit'),
				'type'      => Controls_Manager::SLIDER,
				'range'     => [
					'px' => [
						'min'  => -360,
		...
		$element->add_group_control(
			Group_Control_Border::get_type(),
			[
				'name'     => 'roavio_icon_border',
				'label'    => esc_html__('Border', 'roavio-toolkit'),
				'selector' => '{{WRAPPER}} .e-n-tab-icon',
			]
		);

		$element->add_responsive_control(
			'roavio_icon_border_radius',
			[
				'label'      => esc_html__('Border Radius', 'roavio-toolkit'),
				'type'       => Controls_Manager::DIMENSIONS,
	...
	{
		$element->start_controls_section(
			'roavio_acc_item_style_section',
			[
				'label' => esc_html__('Accordion Item', 'roavio-toolkit'),
		...
		$element->start_controls_tabs('roavio_acc_item_tabs');

		$element->start_controls_tab(
			'roavio_acc_item_normal_tab',
			[
		...
		$element->add_group_control(
			Group_Control_Box_Shadow::get_type(),
			[
				'name'     => 'roavio_acc_item_hover_shadow',
				'selector' => '{{WRAPPER}} .e-n-accordion-item:not([open]):hover',
			]
		);
		...
		$element->start_controls_tab(
			'roavio_acc_item_active_tab',
			[
				'label' => esc_html__('Active', 'roavio-toolkit'),
			]
		);
		...
		$element->add_group_control(
			Group_Control_Background::get_type(),
			[
				'name'     => 'roavio_acc_item_active_bg',
				'types'    => ['classic', 'gradient'],
				'exclude'  => ['image'],
		...
		$element->add_group_control(
			Group_Control_Box_Shadow::get_type(),
			[
				'name'     => 'roavio_acc_item_active_shadow',
				'selector' => '{{WRAPPER}} .e-n-accordion-item[open]',
			]
		);
...
Roavio_Elementor_Extender::instance();
