            ...
            <div class="row justify-content-center">
                <?php if ('cpt' == $settings['portfolio_type']) : ?>
                    ...
                    if ('categories' == $settings['post_from'] && $settings['cat_slugs']) {
                        $args['tax_query'] = [
                            [
                                'taxonomy' => 'roavio_portfolio_category',
                                'field'    => 'slug',
                    ...
                    while ($wp_query->have_posts()): $wp_query->the_post();
                        $idd             = get_the_ID();
                        $categories_list = get_the_terms($idd, 'roavio_portfolio_category', '', '', '');

                        $portfolio_meta = get_post_meta($idd, 'roavio_portfolio_meta', true);
                                    ...
                                    <?php if ('yes' === $settings['show_category']) : ?>
                                        <div class="tags">
                                            <?php if (!empty($categories_list[0]->name)) : ?>
                                                <a href="<?php echo esc_url(get_term_link($categories_list[0]->term_id, 'roavio_portfolio_category')); ?>"><?php echo esc_html($categories_list[0]->name); ?></a>
                                            <?php endif; ?>
                                            <?php if (!empty($categories_list[1]->name)) : ?>
                                                <a href="<?php echo esc_url(get_term_link($categories_list[1]->term_id, 'roavio_portfolio_category')); ?>"><?php echo esc_html($categories_list[1]->name); ?></a>
                                            <?php endif; ?>
                                        </div>
                                    <?php endif; ?>
