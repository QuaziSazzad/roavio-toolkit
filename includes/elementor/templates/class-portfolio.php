...
namespace RoavioToolkit\ElementorAddon\Templates;
...
class Portfolio
{
	...
	{
		$wrapper = ['roavio-portfolio-items'];

		if ('carousel' == $settings['layout']) {
			$wrapper[] = 'roavio-carousel-wrapper';
		}
...
?>
		<div class="<?php echo esc_attr(implode(' ', $wrapper)) ?>">
			...
			<?php elseif ('carousel' == $settings['layout']) : ?>
				<div class="roavio-carousel-active">
					<?php $this->render_loop($settings); ?>
				</div>
	...
	public function render_loop($settings)
	{
		...
		while ($wp_query->have_posts()): $wp_query->the_post();
			...
			elseif ('carousel' == $settings['layout']) : ?>
				<div class="roavio-carousel-item">
	...
	{
		$idd             = get_the_ID();
		$categories_list = get_the_term_list($idd, 'roavio_portfolio_category', '', '', '');
		?>
