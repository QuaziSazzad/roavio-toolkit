...
class Template_Frontend
{
	...
	public function __construct()
	{
		add_action('wp', function () {
			...
			add_action('roavio_builder_before_main', [$this, 'header'], 5);

			if ('roavio_template' !== get_post_type()) {
				add_action('roavio_builder_after_main', [$this, 'popup'], 5);
			}

			add_action('roavio_builder_after_main', [$this, 'back_to_top'], 5);

			add_action('roavio_builder_after_main', [$this, 'footer'], 5);
		});

		add_shortcode('roavio-tb-block', [$this, 'blocks_shortcode']);
	}
	...
	{
		$back_to_top = get_post_meta(get_the_ID(), 'roavio_page_meta', true);
...
?>
	...
	public function display($type)
	{
		if ($type === 'header') {
			...
			if ($id) {
			?>
				<header class="roavio-site-header">
		...
		if ($type === 'footer') {
			...
			if ($id) {
			?>
				<footer class="roavio-site-footer">
					<?php echo self::get_elementor_content($id); ?>
				</footer>
		<?php
			}
	...
	{
		$meta            = get_post_meta($id, 'roavio_tb_settings', true);
		$wrapper_style   = '';
		$container_style = '';
		$overly_color    = '';
		$close_style     = '';
		...
		if ($editing) {
			$wrapper_class = 'roavio-popup-wrapper show-popup editing';
		} else {
			$wrapper_class = 'roavio-popup-wrapper';
		}
