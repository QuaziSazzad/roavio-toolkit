...
class Template_Rule
{
			...
			foreach ($rules as $key => $rule) {
				...
				switch ($rule_case) {
					...
					case 'portfolio_details':
						if (is_singular('roavio_portfolio')) {
							$hide = true;
						}
						break;
	...
	public function get_current_page_type()
	{
		if (null === self::$current_page_type) {
			...
			if (is_front_page()) {
				...
				$current_id = get_the_id();
			} elseif (is_singular('roavio_portfolio')) {
	...
	public function get_templates_by_condition()
	{
		...
		foreach ($templates as $local_template) {
			$meta = get_post_meta($local_template->ID, 'roavio_tb_settings', true);
