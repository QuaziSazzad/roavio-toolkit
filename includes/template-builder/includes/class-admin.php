...
class Template_Admin
{
	...
	public function __construct()
	{
		...
		add_filter('manage_roavio_template_posts_columns', [$this, 'custom_columns']);
		add_filter('manage_roavio_template_posts_custom_column', [$this, 'display_custom_columns']);
		add_filter('nav_menu_items_roavio_template', [$this, 'filter_mega_menu_in_nav']);
		add_filter('nav_menu_items_roavio_template_recent', [$this, 'filter_mega_menu_in_nav']);

		// Add a label to menu area
		add_filter('wp_setup_nav_menu_item', function ($menu_item) {
			if ($menu_item->object === 'roavio_template') {
				$menu_item->type_label = esc_html__('Template Builder Menu', 'roavio-toolkit');
			}
	...
	{
		add_menu_page(
			__('Template Builder', 'roavio-toolkit'),
			__('Template Builder', 'roavio-toolkit'),
			'manage_options',
			'edit.php?post_type=roavio_template',
			'',
	...
	{
		$columns['type'] = __('Type', 'roavio-toolkit');
		$columns['info'] = __('Info', 'roavio-toolkit');
	...
	{
		$meta = get_post_meta($post_id, 'roavio_tb_settings', true);
	...
	public function get_item_info($post_id)
	{
		...
		if ($type == 'block') {
			$info = '<input class="wp-ui-text-highlight code widefat" type="text" onfocus="this.select();" readonly="readonly" value="[roavio-tb-block id=&quot;' . $post_id . '&quot;]">';
		} elseif ($type === 'offcanvas') {
			$settings = get_post_meta($post_id, 'roavio_tb_settings', true);
			$info     .= '<b>' . esc_html('Width:', 'roavio-toolkit') . '</b> ' . $settings['offcanvas_width']['width'] . 'px';
		} else {
	...
	{
		$info    = null;
		$include = get_post_meta($post_id, 'roavio_tb_' . $type, true);
