...
{
	$items = [];
	$lists = ['0' => __('--- Select Template ---', 'roavio-toolkit')];
 ...
 * Post share links
 */
function roavio_post_share_links()
{
	global $post;
	...
	$share_items       = Roavio_Helper::get_option('social_share_item', [
		'enabled' => [
			'facebook' => 'Facebook',
			'twitter'  => 'Twitter',
			'linkedin' => 'Linkedin',
...
if (! function_exists('rt_cpt_per_page')) {
	...
	function rt_cpt_per_page($query)
	{
		...
		if ($query->is_post_type_archive('roavio_portfolio') && ! is_admin() && $query->is_main_query()) {
			$post_per_page = Roavio_Helper::get_option('portfolio_post_per_page', '9');
...
if (!function_exists('roavio_elementor_style_options')) :
	function roavio_elementor_style_options($agrs, $label, $selector, $condition, $style = 'color', $typo = true, $color = true)
	{
		...
		if ($style == 'background-color') :
			if ($color) :
				$agrs->add_control(
					...
					[
						'label' => __('Background Color', 'roavio-addon'),
...
if (!function_exists('roavio_elementor_button_style_options')) :
	function roavio_elementor_button_style_options($init, $label, $selector, $hover_bg_selector = '', $condition = 'layout_one')
	{
		//Label
		$init->add_control(
		...
		$init->add_control(
			...
			[
				'label' => __('Border Radius', 'roavio-addon'),
				'type' => \Elementor\Controls_Manager::DIMENSIONS,
				'size_units' => ['px', '%'],
				'selectors' => [
		...
		$init->add_control(
			str_replace(' ', '_', $label) . '_color',
			[
				'label' => __('Text Color', 'roavio-addon'),
				'type' => \Elementor\Controls_Manager::COLOR,
				'default' => '',
				'selectors' => [
		...
		$init->add_control(
			...
			[
				'label' => __('Border Color', 'roavio-addon'),
				'type' => \Elementor\Controls_Manager::COLOR,
				'condition' => [
